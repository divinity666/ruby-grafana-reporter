...
describe ConsoleConfigurationWizard do
  context 'config wizard' do
    ...
    it 'creates config file properly' do
      ...
      expect(YAML.load_file(config_file)).to eq({'grafana' => {'default' => {'host' => @config[1].strip, 'api_key' => STUBS[:key_admin]}}, 'grafana-reporter' => {'report-class' => 'GrafanaReporter::Asciidoctor::Report', 'templates-folder' => folder, 'reports-folder' => '.', 'report-retention' => 24, 'webservice-port' => 8815}, 'default-document-attributes' => {'imagesdir' => '.'}})
    end
